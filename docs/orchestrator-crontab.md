...
| 3 | 入れ替え・停滞判定時のデーモン制御ソケット経由の明示的終了 | 古い個体と新しい個体の並走 |
| 4 | 巡回のたびに余分なセッションを終了する（`reconcileDuplicates`） | 3層目をすり抜けた個体 |

1層目だけでは、状態ファイルが消えたときに2つのループが同じイシューを取り合う。

**4層目は通知ではなく収束させる処理である。** 3層目が漏らした個体を報せるだけにすると、
その個体は永久に残る。`liveOrchestrators` は新しい順に並べるので、以後の判定（入れ替え・
停滞・指示ファイル反映）は全て最新セッションに当たり、取り残された古い個体は二度と終了
対象にならない。文脈の上限を超えたセッションが、後継の小さい文脈を測られながら「正常」と
判定されて巡回し続ける。

したがって `container-sweep` と同じ扱いにする。状態を毎回読み、証拠で判定し、取りこぼしは
次の巡回で拾う。

- 残すのは**最新**。supervisor が意図して起動した個体であり、取り残されるのは入れ替えの
  対象だった消耗済みの個体のほうである
- 落とすのは**古いと証明できるものだけ**。`startedAt` が無ければ順序が決まらず、`id` が
  無ければ宛先が無い。どちらも殺さずに報告へ回す
- 子エージェントはセッションの死を越えて生き残るので、進行中の作業は失われない
- 全て終了できた回は**通知しない**。人間が手を出す必要が無いため。stdout には残す

**終了は「受理」ではなく「消えたこと」で確かめる。** デーモンが kill を受理しても、常駐は
ターンの途中なら即座には落ちない。受理を成功と読むと、supervisor は後継を起動したうえで
二度と確認しないため、そこで二重稼働が固定される。`terminateSessionAndWait` はエージェント
一覧から消えるまで確認し、消えなければ落ちなかったものとして報告する。
