...
**終了は「受理」ではなく「もう走っていないこと」で確かめる。** デーモンが kill を受理しても、
常駐はターンの途中なら即座には落ちない。受理を成功と読むと、supervisor は後継を起動したうえで
二度と確認しないため、そこで二重稼働が固定される。`terminateSessionAndWait` は、デーモンが
そのジョブを手放したか、エージェント一覧から**消えるか、終端状態になるか**まで確認し、
そうならなければ落ちなかったものとして報告する。

**一覧が読めなかった回を成功に数えない。** `listAgents` は実行失敗を空の一覧に均すので、
不在の判定に使うと「デーモンが居ないと言った」と「デーモンが答えなかった」が同じ顔になる。
待機側は `tryListAgents` を使い、読めなかった回は判断せず次の確認へ送る。

## 生きているかはデーモンに聞く

**ジョブの `state` は、そのセッションが自分について最後に書いた値であって、生存確認ではない。**
終端状態を書かずに死んだセッション（kill、クラッシュ、デーモン再起動による孤児）は、居た状態を
永久に主張する。

`blocked` はこの中で最も見分けが付かない。セッションが何日でも正当に留まりうる状態なので古さは
異常ではなく、定義上その間 `updatedAt` も動かないので更新の途絶えも異常ではない。行から
手掛かりが得られない。

デーモンはプロセスの持ち主なので、**デーモンが持っていないジョブは走っていない**。読み取り専用の
`op: "list"`（`listDaemonJobs`）で保持中の short id を引き、これを生存の根拠にする。

- **逆は成り立たない。** 終了したジョブもしばらく一覧に残るので、保持されていること自体は
  何も証明しない。根拠になるのは不在のほうだけ
- **全デーモンが答えたときだけ集合を採る。** 黙ったデーモンが持つジョブは集合に入らず、不在は
  死と読まれる。走っているオーケストレータが `liveOrchestrators` から落ち、その隣に2本目が
  起動する。部分的な答えは部分的な真実ではなく誤りなので、答えが無いのと同じ扱いにする
- **聞けなかったときは `state` の言い分を採る**（`isRunningAgent`）。short id を持たない対話
  セッションはデーモンのジョブではないので、不在は証拠にならない。機能が無効になる側は安全側で、
  誤って死んだと判定する側は安全側ではない
- 応答は改行区切りで、ジョブ一覧は大きくなる。**改行まで蓄えてから解析する。** 分割された応答を
  取りこぼすと「そのデーモンは答えなかった」と同じ顔になる
